...
const FilePreviewModal: React.FC<FilePreviewModalProps> = ({
  ...
  const handleDownload = useCallback((e: React.MouseEvent) => {
    ...
    try {
      ...
      setAriaLiveMessage(`Downloaded ${currentFile.name}`);
  ...
  const handleDragStart = useCallback((e: React.DragEvent, index: number) => {
    ...
    setAriaLiveMessage(`Picked up image ${index + 1} of ${allFiles.length}`);
  }, []);
  ...
  const handleThumbnailKeyDown = useCallback((e: React.KeyboardEvent, index: number) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowRight':
        if (keyboardPickedUpIndex !== null) {
          e.preventDefault();
          const newIndex = Math.min(allFiles.length - 1, keyboardPickedUpIndex + 1);
          if (newIndex !== keyboardPickedUpIndex) {
            onReorder(keyboardPickedUpIndex, newIndex);
            setKeyboardPickedUpIndex(newIndex);
            setCurrentIndex(newIndex);
  ...
  }, [keyboardPickedUpIndex, onReorder, setCurrentIndex, allFiles.length]);
        ...
        <div className="flex-shrink-0">
          ...
          {onDelete && (
            <button
              ...
              title={`Delete image ${currentIndex + 1} of ${allFiles.length}`}
              type="button"
              aria-label={`Delete image ${currentIndex + 1} of ${allFiles.length}`}
            >
                  ...
                  <div className="relative group w-full h-full flex items-center justify-center">
                    <div className="max-w-full max-h-full overflow-hidden">
                      ...
                      <div className={`absolute inset-0 ring-2 transition-opacity duration-200 rounded-lg ${
                        activeFileId === currentFile.id ? 'ring-primary opacity-100' : 'ring-transparent opacity-0'
                      }`} />
                    </div>
                  </div>
                )}
              ...
              >
                {allFiles.map((f, index) => (
                  <div
                    ...
                    aria-label={`Preview ${f.name}, image ${index + 1} of ${allFiles.length}${index === currentIndex ? ', currently active' : ''}`}
